...
R_chann_weights = np.random.normal(0.0, pow(RGB_kernels_size * RGB_kernels_size, -0.5),
                                   (RGB_kernels_count, RGB_kernels_size, RGB_kernels_size))
G_chann_weights = np.random.normal(0.0, pow(RGB_kernels_size * RGB_kernels_size, -0.5),
                                   (RGB_kernels_count, RGB_kernels_size, RGB_kernels_size))
B_chann_weights = np.random.normal(0.0, pow(RGB_kernels_size * RGB_kernels_size, -0.5),
                                   (RGB_kernels_count, RGB_kernels_size, RGB_kernels_size))
    ...
    for s in range(kernels_count):
        for h in range(pool_size):
            for w in range(pool_size):
                pool_part = conv_layer[s, h * pool_part_size:h * pool_part_size + pool_part_size,
                            w * pool_part_size:w * pool_part_size + pool_part_size]
                pooling_layer[s, h, w] = conv_layer[s, h * pool_part_size:h * pool_part_size + pool_part_size,
...
def back_prop(pl_error_pattern, pl_error_wrong_count, prev_pool_size, conv_layer_size, conv_layer_error, kernels, weights_rot_180, kernel_size):
    for i in range(kernels):
        pl_error_pattern[i, kernel_size - 1:conv_layer_size + kernel_size - 1,
        kernel_size - 1:conv_layer_size + kernel_size - 1] = conv_layer_error[i]  # Матрица ошибок нужного размера для прогона по ней весов
    ...
    for s in range(kernels):
        for h in range(prev_pool_size):
            for w in range(prev_pool_size):
                pl_error_wrong_count[s, h, w] = np.sum(
                    pl_error_pattern[s, h:h + kernel_size, w:w + kernel_size] * weights_rot_180[s])
    ...
    for i in range(kernels_count):
        for h in range(kernels_size):
            for w in range(kernels_size):
                kernel_weights[i, h, w] -= np.sum(
                    conv_layer_error[i] * input_layer[h:h + conv_layer_size, w:w + conv_layer_size] * learning_rate)
    return kernel_weights

@njit
def weights_updating(prev_kernels_count, kernel_weights, kernels_size, kernel_per_input, input_layer, conv_layer_error,
                     conv_layer_size):
    l = 0
    r = kernel_per_input
    for k in range(prev_kernels_count):
        for i in range(l, r):
            for h in range(kernels_size):
                for w in range(kernels_size):
                    kernel_weights[i, h, w] -= np.sum(conv_layer_error[i] * input_layer[k, h:h + conv_layer_size,
                                                                            w:w + conv_layer_size] * learning_rate)
        l = r
        r += kernel_per_input
    return kernel_weights
...
def training(img, targets, R_chann_weights, G_chann_weights, B_chann_weights, output_weights, kernel2_weights, kernel3_weights):
    ...
    conv_layer2 = np.maximum(next_layers_convolution(conv_layer2, conv_layer2_size, pooling_layer1, kernel2_weights, RGB_kernels_count,
    ...
    conv_layer3 = np.maximum(
        next_layers_convolution(conv_layer3, conv_layer3_size, pooling_layer2, kernel3_weights, kernels_2layer_count,
                                kernels_3layer_size, (kernels_3layer_count // kernels_2layer_count)), 0)
    ...
    pl2_error_wrong_count = back_prop(pl2_error_pattern, pl2_error_wrong_count, pool_size2, conv_layer3_size,
                                      conv_layer3_error, kernels_3layer_count, kernel3_weights, kernels_3layer_size)
    pooling_layer2_Error = pooling_error_convertation(pooling_layer2_Error, pl2_error_wrong_count, kernels_2layer_count,
                                                      (kernels_3layer_count // kernels_2layer_count), pooling_layer2)
    ...
    pl1_error_wrong_count = back_prop(pl1_error_pattern, pl1_error_wrong_count, pool_size1, conv_layer2_size,
                                      conv_layer2_error, kernels_2layer_count, kernel2_weights, kernels_2layer_size)
    pooling_layer1_Error = pooling_error_convertation(pooling_layer1_Error, pl1_error_wrong_count, RGB_kernels_count,
                                                      (kernels_2layer_count // RGB_kernels_count), pooling_layer1)
    ...
    kernel3_weights = weights_updating(kernels_2layer_count, kernel3_weights, kernels_3layer_size,
                                       (kernels_3layer_count // kernels_2layer_count), conv_layer2, conv_layer3_error, conv_layer3_size)
    kernel2_weights = weights_updating(RGB_kernels_count, kernel2_weights, kernels_2layer_size,
                                       (kernels_2layer_count // RGB_kernels_count), conv_layer1, conv_layer2_error, conv_layer2_size)
...
for i in range(epochs):
    for j in tqdm(random_inputs_names, desc=str(i + 1)):
        ...
        targets = np.array([0.01, 0.01, 0.01, 0.01, 0.01]).reshape((5, 1))

        if str(j[0]) == 'D' and str(j[1]) == 'P':
            targets[0] = 0.99  # 1
        elif str(j[0]) == 'K' and str(j[1]) == 'P':
            targets[1] = 0.99  # 1
        elif str(j[0]) == 'P' and str(j[1]) == 'O':
            targets[2] = 0.99  # 1
        elif str(j[0]) == 'P' and str(j[1]) == 'P':
            targets[3] = 0.99  # 1
        elif str(j[0]) == 'V' and str(j[1]) == 'P':
            targets[4] = 0.99  # 1
...
def testing(img):
    ...
    conv_layer3 = np.maximum(next_layers_convolution(conv_layer3, conv_layer3_size, pooling_layer2, kernel3_weights, kernels_2layer_count,
                                kernels_3layer_size, (kernels_3layer_count // kernels_2layer_count)), 0)
...
for j in tqdm(os.listdir(path)):
    ...
    if (max_output_index == targets_ind):
        efficiency.append(1)
        efficiency1.append(max_output_index)
    else:
        efficiency.append(0)
        efficiency1.append(max_output_index)

efficiency_array = np.asarray(efficiency)
efficiency1_array = np.asarray(efficiency1)
...
np.savetxt("efficiency1111_telo_arr.txt", efficiency_array, delimiter=",")
np.savetxt("efficiency11111_telo_arr.txt", efficiency1_array, delimiter=",")

with open('weights2/R_conv_layer.csv', 'w') as outfile:
