...
def recognize():
    ...
    if (max_output_index == 0):
        TEXT = main1.recognize(path)
        toc = time.perf_counter()
        TIME = toc - tic
        TIME = f"{TIME:0.4f}"
        text = "Название файла: " + path + " \n Время классификации: " + TIME + " \n Размер изображения 300x300" + "\n Результат распознавания: "
        print("На данном изображении нет перелома \n" + TEXT)
        ui.label.setText(text + "На данном изображении нет перелома \n" + TEXT)
    elif (max_output_index == 1):
        toc = time.perf_counter()
        TIME = toc - tic
        TIME = f"{TIME:0.4f}"
        text = "Название файла: " + path + " \n Время классификации: " + TIME + " \n Размер изображения 300x300" + "\n Результат распознавания: "
        print(text + "На данном изображении нет рентгена")
        ui.label.setText(text + "На данном изображении нет рентгена")
    elif max_output_index == 2:
        TEXT = main1.recognize(path)
        TEXT1 = main2.recognize(path)
        toc = time.perf_counter()
        TIME = toc - tic
        TIME = f"{TIME:0.4f}"
        text = "Название файла: " + path + " \n Время классификации: " + TIME + " \n Размер изображения 300x300" + "\n Результат распознавания: "
        print(text + "На данном изображении есть перелом \n" + TEXT + "\n" + TEXT1)
        ui.label.setText(text + "На данном изображении есть перелом \n" + TEXT + "\n" + TEXT1)
