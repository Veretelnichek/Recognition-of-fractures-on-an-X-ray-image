...
for i in range(epochs):
    for j in tqdm(random_inputs_names, desc=str(i + 1)):
        ...
        targets = np.array([0.01, 0.01, 0.01]).reshape((3, 1))

        if str(j[0]) == 'C' and str(j[1]) == 'H':
            targets[0] = 0.99  # 1
        elif str(j[0]) == 'K' and str(j[1]) == 'O':
            targets[1] = 0.99  # 1
        elif str(j[0]) == 'T' and str(j[1]) == 'O':
            targets[2] = 0.99  # 1
...
np.savetxt("efficiency11_telo_arr.txt", efficiency_array, delimiter=",")
np.savetxt("efficiency111_telo_arr.txt", efficiency1_array, delimiter=",")

with open('weights1/R_conv_layer.csv', 'w') as outfile:
